...
public:
    ...
    bool ShouldDelContent(const bool& parseSuccess, const std::string& sourceKey, const bool& sourceKeyOverwritten) {
        return parseSuccess && !sourceKeyOverwritten && sourceKey != mRenamedSourceKey;
    }

    bool ShouldAddRenamedSourceLog(const bool& parseSuccess, const std::string& sourceKey) {
        return mKeepingSourceWhenParseSucceed
            && ((parseSuccess && sourceKey != mRenamedSourceKey) || (!parseSuccess && mKeepingSourceWhenParseFail));
    }
};
